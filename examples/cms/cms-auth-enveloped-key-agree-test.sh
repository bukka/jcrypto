...
jcrypto_out_enc_file="$jcrypto_this_dir/out-cms-auth-env-kye-agree-enc.pem"
jcrypto_out_plain_file_from_jcrypto="$jcrypto_this_dir/out-cms-auth-env-kye-agree-plain-jc.txt"
jcrypto_out_plain_file_from_ossl="$jcrypto_this_dir/out-cms-auth-env-kye-agree-plain-ossl.txt"
jcrypto_out_ossl_enc_file="$jcrypto_this_dir/out-cms-auth-env-kye-agree-ossl-enc.pem"
jcrypto_out_ossl_plain_file_from_jcrypto="$jcrypto_this_dir/out-cms-auth-env-kye-agree-ossl-plain-jc.txt"
jcrypto_out_ossl_plain_file_from_ossl="$jcrypto_this_dir/out-cms-auth-env-kye-agree-ossl-plain-ossl.txt"

echo "JCRYPTO ENCRYPT"
jcrypto cms encrypt -i "$jcrypto_this_dir/in-cms-data.txt" -c aes-128-gcm \
...
jcrypto_openssl cms -encrypt -in "$jcrypto_this_dir/in-cms-data.txt" -out "$jcrypto_out_ossl_enc_file" \
  -outform PEM -recip "$jcrypto_recip_cert_file" -aes-128-gcm
...
#  -originator "$jcrypto_orig_cert_file" -aes-128-gcm
jcrypto_dump_pem "$jcrypto_out_ossl_enc_file"

echo "OPENSSL DECRYPT OF OPENSSL"
jcrypto_openssl cms -decrypt -in "$jcrypto_out_ossl_enc_file" -inform PEM -aes-128-gcm \
...
jcrypto_openssl cms -decrypt -in "$jcrypto_out_enc_file" -inform PEM -aes-128-gcm \
...
jcrypto cms decrypt -i "$jcrypto_out_enc_file" -f PEM -c aes-128-gcm -o "$jcrypto_out_plain_file_from_jcrypto" \
...
jcrypto cms decrypt -i "$jcrypto_out_ossl_enc_file" -f PEM -c aes-128-gcm -o "$jcrypto_out_plain_file_from_ossl" \
	--recipient-cert "$jcrypto_recip_cert_file" --private-key "$jcrypto_recip_priv_key"
