...
jcrypto_pub_key_file="$jcrypto_tmp_dir/out-pkey-ec-${jcrypto_curve_name}-pub-key-pkcs11.der"
...
jcrypto pkey generate --public-key-file "$jcrypto_pub_key_file" --private-key-alias JavaTestECKey \
...
jcrypto_cat "$jcrypto_pub_key_file" | base64

echo "JCRYPTO SIGN"
jcrypto pkey sign -i "$jcrypto_this_dir/in-pkey-data.txt" -a SHA256withECDSA -o "$jcrypto_out_sig_file" \
  --private-key-alias JavaTestECKey --key-store-password 1234 --key-store-name PKCS11 \
  --provider-name SunPKCS11 --provider-config-file "$jcrypto_pkcs11_java_config"

echo "JCRYPTO VERIFY"
jcrypto pkey verify -i "$jcrypto_this_dir/in-pkey-data.txt" -a SHA256withECDSA \
  --signature-file "$jcrypto_out_sig_file" --public-key-file "$jcrypto_pub_key_file" \
