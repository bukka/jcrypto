...
jcrypto_out_enc_file="$jcrypto_this_dir/out-smime-env-key-trans-stream-enc.txt"
jcrypto_out_plain_file="$jcrypto_this_dir/out-smime-env-key-trans-stream-plain.txt"

jcrypto mail encrypt -i "$jcrypto_this_dir/in-message-body.txt" -c aes-256-cbc --stream \
...
jcrypto mail decrypt -i "$jcrypto_out_enc_file" -c aes-256-cbc --stream \
