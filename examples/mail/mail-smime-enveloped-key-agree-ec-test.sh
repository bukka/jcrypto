...
jcrypto_out_enc_file="$jcrypto_this_dir/out-smime-env-kye-trans-enc.pem"
jcrypto_out_plain_file_from_jcrypto="$jcrypto_this_dir/out-smime-env-kye-trans-plain-jc.txt"
jcrypto_out_plain_file_from_ossl="$jcrypto_this_dir/out-smime-env-kye-trans-plain-ossl.txt"
jcrypto_out_ossl_enc_file="$jcrypto_this_dir/out-smime-env-kye-trans-ossl-enc.pem"
jcrypto_out_ossl_plain_file_from_jcrypto="$jcrypto_this_dir/out-smime-env-kye-trans-ossl-plain-jc.txt"
jcrypto_out_ossl_plain_file_from_ossl="$jcrypto_this_dir/out-smime-env-kye-trans-ossl-plain-ossl.txt"
...
jcrypto_openssl cms -encrypt -in "$jcrypto_this_dir/in-message-body.txt" -out "$jcrypto_out_ossl_enc_file" \
  ...
  -recip "$jcrypto_recip_cert_file" -inkey "$jcrypto_orig_priv_key" -aes-128-cbc
...
jcrypto_openssl cms -decrypt -in "$jcrypto_out_ossl_enc_file" -inform SMIME -aes-128-cbc \
  -originator "$jcrypto_orig_cert_file" -inkey "$jcrypto_recip_priv_key" -out "$jcrypto_out_ossl_plain_file_from_ossl"
...
jcrypto mail decrypt -i "$jcrypto_out_enc_file" -f PEM -c aes-128-cbc
	--cert "$jcrypto_orig_cert_file" --private-key "$jcrypto_recip_priv_key" -o "$jcrypto_out_plain_file_from_jcrypto"
jcrypto_cat "$jcrypto_out_plain_file_from_jcrypto"

echo "JCRYPTO DECRYPT OF OPENSSL"
jcrypto mail decrypt -i "$jcrypto_out_ossl_enc_file" -f PEM -c aes-128-cbc \
	--cert "$jcrypto_orig_cert_file" --private-key "$jcrypto_recip_priv_key" -o "$jcrypto_out_plain_file_from_ossl"
